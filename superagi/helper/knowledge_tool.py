...
class KnowledgeToolHelper:
    ...
    def pinecone_get_match_vectors(self, query, vector_db_creds, knowledge_details):
        ...
        if knowledge_details['knowledge_vector_db_index_state'] == "MARKETPLACE":
            ...
            search_res = index.query(x_query, top_k = 5, include_metadat = True)
    ...
    def qdrant_get_match_vectors(self, query, vector_db_creds, knowledge_detaiils):
        ...
        try:
            if knowledge_detaiils['knowledge_vector_db_index_state'] == "MARKETPLACE":
                search_res = qdrant_client.search(
                    collection_name = knowledge_detaiils["knowledge_vector_db_index_name"],
                    query_vector = x_query,
                    query_filter = models.Filter(
                    must=[
                        models.FieldCondition(
                          key = 'knowledge_name',
                          match = models.MatchAny(any=knowledge_detaiils["knowledge_name"])
                        )
                    ]
                    ),
                    limit = 5
                )
            elif knowledge_detaiils["knowledge_vector_db_index_state"] == "CUSTOM":
                search_res = qdrant_client.search(
                    collection_name = knowledge_detaiils["knowledge_vector_db_index_name"],
                    query_vector = x_query,
                    limit = 5
